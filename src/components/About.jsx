...
// Dark theme with tool stack icons matching the reference design
// ─────────────────────────────────────────────────────────────
...
const tools = [
  ...
  },
  {
    name: 'Spark',
    icon: (
      <svg viewBox="0 0 24 24" fill="#88f3e7" className="w-5 h-5">
    ...
    >
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
          ...
          >
            <h2 className="font-display font-bold text-[clamp(2rem,4vw,3.5rem)] leading-[1.1] tracking-tight text-[#6ba8a3] mb-4">
              Specialized in
            </h2>
            <h3 className="font-display font-bold text-[clamp(2rem,4vw,3.5rem)] leading-[1.1] tracking-tight text-[#dffcfa] mb-2">
              digital product
            </h3>
            <div className="flex items-end gap-2 mb-12">
              <h3 className="font-display font-bold text-[clamp(2rem,4vw,3.5rem)] leading-[1.1] tracking-tight text-[#dffcfa]">
                motion.
              </h3>
              <motion.span
                animate={{ opacity: [0.3, 1, 0.3] }}
                transition={{ duration: 1.5, repeat: Infinity }}
                className="w-3 h-3 rounded-full bg-[#88f3e7] mb-4"
              />
            </div>

            {/* Tool Stack */}
            <div>
              <p className="text-[#6ba8a3] text-sm font-medium mb-4">My tech stack</p>
              <div className="flex flex-wrap gap-3">
                {tools.map((tool) => (
                  <motion.div
                    key={tool.name}
                    whileHover={{ y: -2, borderColor: 'rgba(136, 243, 231, 0.3)' }}
                    transition={{ duration: 0.2 }}
                    title={tool.name}
                    className="tool-card"
                    style={{
                      background: tool.bgColor || '#0d1a1c',
                      borderColor: 'rgba(136, 243, 231, 0.1)',
                    }}
                  >
                    {tool.icon ? (
                      tool.icon
                    ) : (
                      <span style={{ color: tool.textColor }} className="font-bold text-sm">
                        {tool.abbr}
                      </span>
                    )}
                  </motion.div>
          ...
          >
            {services.map((service, index) => (
              ...
              >
                ...
                <span className="text-[#dffcfa] font-semibold text-lg">
                  {service.label}
                </span>
              </motion.div>
            ))}
        ...
        >
          <h2 className="font-display text-[clamp(1.8rem,4vw,3.5rem)] leading-[1.1] tracking-tight mb-6">
            <span className="text-[#6ba8a3] font-medium italic">Want similar</span>
            <br />
            <span className="text-[#dffcfa] font-bold">Motion design for your</span>
            <br />
            <span className="text-[#dffcfa] font-bold">brand?</span>
          </h2>

          <div className="max-w-lg mx-auto mb-8">
            <p className="text-[#dffcfa] font-semibold text-lg">
              Get a dedicated motion partner{' '}
              <span className="text-[#6ba8a3] font-normal">
          ...
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="#88f3e7" strokeWidth="2" className="w-5 h-5">
